...
RT_PROGRAM void closest_hit() {
    ...
    const optix::float3 shading_normal = nbuf[n_idx.y] * barycentrics.x +
                                         nbuf[n_idx.z] * barycentrics.y +
    ...
    thePayload.radiance =
        optix::normalize(rtTransformNormal(RT_OBJECT_TO_WORLD, shading_normal)) * 0.5f + 0.5f;
}
