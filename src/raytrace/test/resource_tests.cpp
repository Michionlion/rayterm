...
TEST(GeometryTest, ObjLoad) {
    ...
    uint64_t num_verts = mesh->attrib.vertices.size() / 3;
...
TEST(GeometryTest, MatLoad) {
    ...
    uint64_t num_vars = mat->getVariableCount();
...
TEST(GeometryTest, GeometryInstance) {
    ...
    uint64_t num_vars = instance->getVariableCount();
    EXPECT_EQ(num_vars, 4) << "Did not load buffer and material variables";

    uint64_t num_mats = instance->getMaterialCount();
